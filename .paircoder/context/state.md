...
- **TB.12 done (2026-05-13)** — Error handling (toast + retry). Two real gaps closed; the other two views already met the AC. **Gap 1 — `src/ui/quiz-view.ts`** had no toast on any of its three AI-call paths (`startQuiz` / `runMoreQuestions` / `runRegenerate`) and the existing Retry button routed back to the hub rather than re-running the failed call. Refactored `renderError(state, message, onRetry)` to take a retry callback. Each call site now passes its own retry closure (re-invokes the same async fn with the same args) and fires `showToast(state.toastContainer, "<context> failed: <message>", 'error')` after rendering the error block. The "no API key" branches still call `renderError` but with `() => renderHub(state)` as the retry (no AI call happened → no toast needed — matches the summary/flashcards "no key" treatment). **Gap 2 — `src/ui/teachback-view.ts` `renderResultError`** rendered the inline error block inside `.teachback__result-container` but didn't include a Retry button. The pane-wide `renderTeachbackError` (no-key empty state) already had one — this was only the result-area variant. Added the Retry button (`data-role="teachback-result-retry"`) and refactored the submit wiring: extracted a `runEvaluation(explanation)` closure inside `wireSubmit` that owns the loading → success/error → toast + retry-wiring lifecycle; the original submit handler now reads the textarea, validates, and calls `runEvaluation`; the failure branch also wires the retry button to `runEvaluation(explanation)` so it re-submits with the **same explanation** captured at submit time (NOT a fresh read of the textarea — proves "retry uses the same call" even if the user has since cleared the field). **Summary view + Flashcards view were already compliant** — both had `showToast(options.toastContainer, "<Mode> failed: <message>", 'error')` in their catch blocks and their retry buttons re-call `showSummary` / `showFlashcards` which re-enters `loadX` (under `withGenerationCache` → same cache key `summary_<chapterId>` / `flashcards_<chapterId>` because no row was written on the failed first attempt). Only added new tests for these to lock the contract. **AC #4 ("Retry uses the same cache key") verification strategy** — after each retry, the test imports `getCachedGeneration` from `src/lib/cache.ts` and asserts `getCachedGeneration<T>('<type>', chapterId)` returns the post-retry value. Since the cache layer keys rows by `${type}_${chapterId}` (locked TB.4 contract), a successful read at this key after retry proves both the retry hit the same cache key AND the writeback used the same key — no key mutation across the failure→success boundary. **AC #3 ("Vitest fakes a 500 and asserts toast") landed in every mode test** — `mockResolvedValue` (not `Once`) returns a 500 for every fetch attempt, the SDK exhausts its built-in 5xx retry budget (~1.2s wall-time per test — same shape as the existing `anthropic.test.ts` "throws on 5xx" test), the eventual error is wrapped by `wrapError` and the view's catch block surfaces it as a toast. Each 500 test uses `{ timeout: 10_000 }` per-test to cover the SDK's retry backoff. The retry-re-runs tests use **400** (non-retryable, single-call failure) chained with a 200 success — needed to make the first-failure/second-success pairing deterministic without fighting the SDK retry loop. **New tests (8 total): `summary-view.test.ts` × 2** (500 toast; retry re-runs + same cache key). **`flashcards-view.test.ts` × 2** (same pair). **`teachback-view.test.ts` × 2** (500 toast on result-area; result-area Retry re-submits + same cache key + explanation persists). **`quiz-view.test.ts` × 2** (a 400 Start failure → toast + Retry that re-runs `loadQuiz` and lands a successful question — proves retry path end-to-end; a 500 Regenerate failure → toast + Retry present). Full suite: **501/501 pass** (was 493). `tsc --noEmit` clean. `bpsai-pair arch check` clean on all 8 touched files (`quiz-view.ts`, `teachback-view.ts`, `summary-view.ts`, `flashcards-view.ts` source + tests). **Spurious working-tree regression on TB.11.task.md flipped from `done` to `failed` at session start** — same engage-lifecycle-hook regression first seen on TA.2 and TB.8. Reverted via `git checkout` before starting; this is the third time this hook has corrupted a task file's status field. **Sprint-B progress: 11 of 12 done; only TB.10 (Format Text dialog, P2 5cx) remains.** Next coding move: `/start-task TB.10` to close Sprint B, then verify the cross-sprint gates (G-AC, G-Tests, G-Arch, G-Security, G-Manual, G-State) and merge `engage/phase-b` → main.

- **TB.11 done** (auto-updated by hook)

- **TB.11 done (2026-05-13)** — Settings UX additions. New `src/ui/settings-provider.ts` (~270 LOC, 12 fns) exports `ProviderState`, `loadProviderState()`, `buildProviderSection(state, doc)`, and `wireProviderSection(section, toastContainer)`. **Key model (AC #1):** key lives only in the `src/data/secrets.ts` in-memory Map — typing into the password input calls `setSecret('anthropicApiKey', value)`; clearing the input calls `clearSecret`. `loadProviderState` reads via `getSecret` and never touches `getSetting`. Per the 2026-05-12 Sprint-B audit P2-4 lock, the key is never written to IDB or localStorage; reload clears it (the `pagehide` listener already in `secrets.ts` belt-and-suspenders this). The dropdown's `selectedModel` value DOES persist in IDB via `setSetting`, but that's the model choice, not the secret. **Show/hide toggle:** button flips `input.type` between `password` and `text` and updates `aria-pressed` + `aria-label` accordingly so screen readers narrate the state change. **Test connection (AC #2):** clicking the Test button calls `fetchAvailableModels(key, {throwOnError: true})` and on success rebuilds the dropdown options from the live list + emits a success toast `"Connected."`, on failure emits an error toast `"Connection failed: <redacted message>"`. The error message is redacted through the same two-layer pass that `callAnthropic` uses — added inside the new `throwOnError` branch of `fetchAvailableModels` itself, so even direct callers get a no-PII contract. **Model dropdown (AC #3):** `<select>` seeded with `FALLBACK_MODELS` (`claude-opus-4-7`, `claude-opus-4-6`, `claude-sonnet-4-6`, `claude-haiku-4-5`) when a key is in memory, empty when no key is set. The dropdown is XSS-safe: option values + label text both enter the DOM as `setAttribute('value', id)` + `createTextNode(id)` — no `innerHTML` path. **Refresh Models (AC #4):** the link calls `fetchAvailableModels(key)` (default behaviour, no throwOnError → silent fallback) and rebuilds the dropdown; only an empty-key click shows a toast. The test asserts the fetch hits `/v1/models` (the SDK's actual endpoint), proving it's a live call and not just reading FALLBACK_MODELS. **API surface change in `src/ai/anthropic.ts`:** added optional `FetchModelsOptions {throwOnError?: boolean}` second arg to `fetchAvailableModels` — backwards compatible (TB.1 contract preserved by default). When throwOnError is true, transport / API errors propagate (with key-redacted messages) and an empty model list also throws. **Wiring in `src/ui/settings.ts`:** new `resolveToastContainer(toastContainer, stack, doc)` helper looks up `.toast-container` in the document, falling back to the modal stack (real-app callers in `src/app.ts` pass the toast container explicitly). The provider section is appended to `modal__body` BEFORE the prompts section. `openSettings` signature now `(stack, doc?, toastContainer?)`; `app.ts` updated to pass `root.querySelector('.toast-container')`. 27 new vitest tests in `src/ui/settings-provider.test.ts` cover: `loadProviderState` × 4 (empty defaults, in-memory key reflection, persisted selectedModel reflection, IDB-never-touched guard for AC #1); `buildProviderSection` × 7 (heading, password input + toggle, Test + Refresh controls, dropdown seeded with Opus/Sonnet/Haiku, selectedModel marked selected, key input seeded with memory key, no-innerHTML XSS guard); `wireProviderSection` key persistence × 2 (typing calls setSecret only, clearing calls clearSecret); show/hide toggle × 1 (type flips back-and-forth); Test Connection × 4 (success populates + toast, failure shows error toast + dropdown unchanged, empty key shows error without API call, key not leaked into failure toast); Refresh Models × 3 (rebuilds dropdown, hits live `/v1/models` endpoint, empty-key short-circuits); model select × 1 (change persists `selectedModel`); FALLBACK_MODELS Anthropic tier guard × 1. 4 new tests in `src/ai/anthropic.test.ts` cover the new throwOnError path (propagates errors, throws on empty list, redacts key from messages, still returns list on success). 1 settings.test.ts heading assertion updated from `['Reading','Data','Prompts']` to `['Reading','Data','AI Provider','Prompts']`. Full suite: **493/493 pass** (was 466). `tsc --noEmit` clean. `npm run build` clean (294 kB JS / 85 kB gz, +4 kB JS / +1 kB gz vs TB.9; 26 kB CSS / 4.8 kB gz, +1 kB CSS for the new section). `bpsai-pair arch check` clean on all 5 touched files. CSS additions in `src/styles.css`: `.settings__section--provider`, `.provider__key-row` (flex layout for input + toggle), `.provider__key-input` (monospace), `.provider__key-toggle` (subtle button, aria-pressed styling), `.provider__actions`, `.provider__model-field`, `.provider__refresh` (underlined accent link). **Spec drift fix carried in:** the navigator pass-#4 commit `b4229c8` updated the backlog but missed `TB.11.task.md` itself; the file still said "Key persists in IDB" + "Gemini Flash + Pro tiers". Fixed as the first step of this task — description + all 4 ACs now match the corrected backlog (memory-only, Anthropic tiers). Next coding move: TB.10 (Format Text dialog, P2 5cx — first consumer of `renderSafeMarkdown` from TB.9) or TB.12 (Error handling, P1 3cx — depends on TB.4, smallest remaining). After both ship, Sprint B is complete.

- **TB.9 done** (auto-updated by hook)

- **TB.9 done (2026-05-13)** — Markdown renderer + sanitizer. New `src/lib/markdown.ts` (~230 LOC, 9 fns) exports `escapeHtml`, `isSafeUrl`, `renderInline`, `renderMarkdown`, `sanitizeHtml`, and `renderSafeMarkdown`. Two-stage defense-in-depth pipeline. **Renderer:** block parser walks lines with a small `BlockState` machine (paraBuf / listType / inBlockquote / inCode), handles `# / ## / ###` headings, bullet (`-` and `*`) + numbered (`N.`) lists, blockquotes (`>` with consecutive-line joining), fenced code blocks (```` ``` ````) with newline preservation + HTML-escape inside, blank-line paragraph separators, and unterminated-fence graceful flush. **Inline:** placeholder-pass approach — extract code spans → extract links → escape the surviving text → bold (`**...**`) → italic (`*...*` and `_..._`) → restore placeholders. Order is load-bearing: code-first prevents `*` inside ``` `*bold*` `` from being parsed; link extraction before escape preserves the URL for `isSafeUrl` validation; placeholders use `\x00` markers so they survive `escapeHtml`. Links emit `target="_blank" rel="noopener"` (AC #4) and route every URL through `isSafeUrl` (allowlist: `http://`, `https://`, `mailto:`, `#`, `/`, `./`, `../`); unsafe URLs (incl. `javascript:`, `data:`, `vbscript:`) fall back to `href="#"`. **Sanitizer:** regex-based tokenizer alternates between text segments (copied as-is; stray `<` re-escaped to `&lt;`) and tag tokens (whitelisted via `WHITELIST_TAGS` of 14 block+inline names, all attributes filtered by per-tag `WHITELIST_ATTRS` — only `<a>` accepts attrs: `href` / `target` / `rel`; everything else drops all attrs). Drops the entire tag for non-whitelisted names (keeps inner text — so `<script>alert(1)</script>` becomes the literal text `alert(1)`); drops disallowed attrs (e.g. `onclick` on `<p>`, `onmouseover` on `<a>`); applies `isSafeUrl` gate to every `href` (so a `<a href="javascript:alert(1)">` round-trip emits `<a target="_blank" rel="noopener">` with the dangerous href dropped); forces `target=_blank` and `rel=noopener` on every anchor that lacks them (existing `rel="nofollow"` is preserved, target still forced). 60 new vitest tests in `src/lib/markdown.test.ts` covering every renderer rule + every XSS fixture + combined pipeline: escapeHtml × 3 (the four special chars, escape-order regression for `&<`, empty input), isSafeUrl × 4 (http/https/mailto, relative/fragment, javascript/data/vbscript rejection w/ case-insensitivity + leading whitespace, file/ftp/empty rejection), renderInline × 11 (bold, italic-star, italic-underscore, code span, no-markdown-inside-code, HTML-escape-inside-code, link with target+rel, javascript:-URL neutralisation, plain-HTML escape, combined bold+italic+code, empty), renderMarkdown headings × 5 (h1, h2, h3, h4-rejected, inline-formatting-in-heading), emphasis × 1, lists × 5 (bullet `-`, bullet `*`, numbered, ul→ol transition, inline-in-list-item), blockquotes × 3 (single-line, multi-line join, blank-line close), code × 6 (inline, fenced, newline preservation, HTML-escape inside, no-markdown-inside, unterminated graceful), links × 2 (full attribute set, count regression on two links), paragraphs × 5 (plain wrap, line join, blank-line separation, empty input, combined doc), sanitizer whitelist × 4 (block tags pass, inline tags pass, disallowed tag drops keeping text, disallowed attr drops), XSS fixtures × 5 (`<script>` removed, `<img onerror>` removed, `<p onerror>` attr stripped, `javascript:` href removed, `onmouseover` stripped from anchor), link attribute forcing × 3 (forces target+rel on bare anchor, preserves existing rel, no-op on already-complete anchor), and combined pipeline × 3 (clean markdown unchanged, end-to-end XSS neutralisation, every link in mixed output gets target+rel). Full suite: **466/466 pass** (was 406). `tsc --noEmit` clean. `npm run build` clean (290 kB JS / 84 kB gz / 25 kB CSS — +13 kB JS / +4 kB gz vs TB.8; the new module is small but currently bundled because nothing tree-shakes pure-function modules unused by app code — TB.10 will be the first consumer when it wires the Format Text dialog). `bpsai-pair arch check` clean on both files. AC #1 (every listed feature renders) verified by 27 renderer-rule tests; AC #2 (XSS fixture neutralised end-to-end) verified by 5 sanitizer XSS tests + 1 combined-pipeline test; AC #3 (vitest covers every renderer rule) verified by the per-rule split; AC #4 (links open in new tab with noopener) verified at three layers — renderInline emits both attributes inline, sanitizeHtml forces them on any anchor that lacks them, and the combined pipeline test asserts every `<a>` in a multi-link output carries them. Known v1 limitations (deferred, no AC pressure): (a) link URL regex does not support balanced parens — `[x](https://a.test/(foo))` would close after `(foo`; test fixture uses paren-free URLs. (b) `_italic_` regex matches inside snake_case identifiers (e.g. `foo_bar_baz` → `foo<em>bar</em>baz`); markdown convention is to use backticks for code-like tokens. (c) numbered lists ignore the actual starting integer — `5. five\n6. six` renders as a standard `<ol>` starting at 1. (d) link extraction now precedes escape, so an inline `[text](url)` whose `text` portion contains literal `<` characters would emit `<a href="...">a<b</a>`; not a problem in practice because the URL allowlist makes the only attack surface the URL itself, which is gated by `isSafeUrl`. Next coding move: TB.10 (Format Text dialog, P2, 5cx — depends only on TB.9, just unblocked) or TB.11 (Settings UX, P1, 5cx — spec corrected last pass) or TB.12 (Error handling, P1, 3cx — depends on TB.4 only). TB.10 is the natural follow-up since it's the first consumer of `renderSafeMarkdown`; TB.11 is the riskiest of the three (real-API touch through `fetchAvailableModels`); TB.12 is the smallest (3cx) and finishes Sprint B.
...
- **TB.8 done (2026-05-12)** — Teach-Back mode. New `src/ai/modes/teachback.ts` (`evaluateTeachback(chapter, explanation, apiKey, model?)` returns `TeachbackResult {strengths, gaps, suggestions, score}`; strict `output_config.format` JSON schema with `score` constrained to integer 0..100 and each bullet array minItems 1 / maxItems 5; defence-in-depth guards throw on empty/whitespace explanation and empty API key before any API call; writes the attempt directly to `STORE_GENERATED` under the locked TB.4 key `teachback_<chapterId>` with content `{explanation, result}` — same direct-`dbPut` pattern as `appendFlashcards`, because `withGenerationCache` only models chapter-only generators and this mode needs to round-trip the user's text alongside the result; `loadCachedTeachback(chapterId)` reads via `getCachedGeneration<TeachbackAttempt>` — no API call). New `src/ui/teachback-view.ts` (~250 LOC, 13 fns; exports `renderTeachback` (form with seeded textarea + Submit + inline validation placeholder), `renderTeachbackLoading`, `renderTeachbackError` (pane-wide no-key empty-state), `renderTeachbackResult` (score badge + 3 sections), and `scoreTier(score)` returning `'low'|'mid'|'high'` — locked tier thresholds: 0–39 low / 40–69 mid / 70–100 high; badge gets `data-tier="<tier>"` attribute + `.teachback__score--<tier>` class, so unit tests can assert tier classification independently of CSS color tokens; `showTeachback` orchestration: no key → pane-wide error+Retry; otherwise read cached attempt → render form (textarea seeded with prior explanation if any), and if cached, immediately mount the result panel from cache with NO API call (AC #3); wire submit click handler that reads textarea via `readExplanation` (prefers `.value`, falls back to `.textContent` — same pattern as `settings-prompts.readTextareaValue`), trims, shows inline validation on empty input WITHOUT firing the API (AC #4), otherwise clears validation, shows loading in the result-container only (form stays put), calls `evaluateTeachback`, and on success replaces the container with `renderTeachbackResult`; on failure renders an inline `.teachback__result-error` block AND fires a toast. Inline error state is intentionally distinct from the pane-wide error: failed submits should NOT wipe the form because the user's explanation is still in the textarea and they may want to revise and resubmit). `src/ui/chapter-view.ts` extended to a four-tab toggle: Read | Summary | Flashcards | Teach-Back (tab list, `switchTab` query list, and event wiring all updated). `src/ai/prompts.ts`: `TEACHBACK_DEFAULT` rewritten from the prior Socratic-session shape (`{openingPrompt, followUps, evaluationRubric}`) to the evaluator shape — the old shape mismatched both the TB.8 backlog AC and the response contract `evaluateTeachback` enforces; new template ships `{title}`/`{content}`/`{explanation}` placeholders and a JSON-shape contract in the prompt body. CSS in `src/styles.css`: `.teachback__score--{low,mid,high}` use `color-mix(in srgb, <token> 18%, var(--panel-2))` for the badge fill so the existing palette tokens (`--error`/`--warn`/`--success`) drive the tier color; `.teachback__section--{strengths,gaps,suggestions}` colorize the section titles (green/amber/blue); `.teachback__validation` reserves `min-height: 1.25rem` so showing/clearing the inline error doesn't reflow neighbouring controls (CLS guard, same pattern as `chapter-view__mark` min-width). 33 new vitest tests: `src/ai/modes/teachback.test.ts` (14 — empty key, empty/whitespace explanation, parse happy path, prompt substitution × {title}/{content}/{explanation}, strict schema enforcement, malformed JSON rejection, missing-score shape rejection, out-of-range score rejection, cache writeback under locked key, resubmit-overwrites-cache, no-cache-on-API-error, loadCachedTeachback empty + present cases) and `src/ui/teachback-view.test.ts` (19 — scoreTier boundaries × 9 (low/mid/high), renderTeachback × 3 (label, controls, seeded textarea), renderTeachbackLoading + renderTeachbackError, renderTeachbackResult × 3 (tier badge, sections present, li count), showTeachback × 8 (no-key error state, empty-form on fresh mount, cache seeds form + result panel — AC #3, empty submit validation no-API — AC #4, whitespace-only submit validation no-API, validation clears after successful submit, full submit→result render — AC #1+#2, API failure → inline `.teachback__result-error`)). Full suite: 377/377 (was 344). `tsc --noEmit` clean. `npm run build` clean (277 kB JS / 81 kB gz; 21.91 kB CSS / 4.27 kB gz — +5 kB JS / +3.4 kB CSS for the new mode). `bpsai-pair arch check` clean on all 6 touched files: `src/ai/modes/teachback.ts` (123 LOC, 6 fns), `src/ai/modes/teachback.test.ts` (~200 LOC), `src/ui/teachback-view.ts` (~250 LOC, 13 fns — under 200-warning is tight but within the < 400 error limit; refactor to extract render helpers if Phase C touches this), `src/ui/teachback-view.test.ts` (~270 LOC), `src/ui/chapter-view.ts` (unchanged structurally — added one tab + one event listener), `src/ai/prompts.ts` (unchanged structure — only TEACHBACK_DEFAULT body replaced). AC #1 (useful feedback — schema enforces strengths/gaps/suggestions/score; manual A/B is reviewer-bench territory and was deferred to manual review per backlog wording), #2 (badge color tiers — locked thresholds verified by 9 unit assertions and exposed as `data-tier` attr for QA hooks), #3 (cache revisit — verified by both AI-layer test ('returns the stored attempt without calling the API') and UI-layer test ('seeds form + result from cache on revisit')), #4 (empty input inline validation — verified by both UI tests (empty + whitespace) and AI-layer defensive guards) all green. Open follow-ups: TB.6 (Quiz mode, P0 13cx) and TB.9 (Markdown + sanitizer, P0 8cx) are next in the dependency graph; TB.6 will adopt the same direct-`dbPut` pattern that flashcards' `appendFlashcards` and teachback now share, since quiz state (per-question attempt history) similarly doesn't fit the chapter-only `withGenerationCache` mold. The four-tab chapter view will become five tabs once TB.6 lands; if a fifth tab regresses readability on narrow viewports, consider a dropdown affordance under 480px.

- **TB.7 done** (auto-updated by hook)

- **TB.7 done (2026-05-12)** — Flashcards mode. New `src/ai/modes/flashcards.ts` (`Flashcard {front, back}`; `generateFlashcardsRaw` calls `callAnthropic` with strict `output_config.format` JSON schema and enforces `cards.length >= 5` in code; `loadFlashcards = withGenerationCache('flashcards', ...)` — fifth adoption of TB.4 pattern; `appendFlashcards` reads cached set via `getCachedGeneration`, calls `generateMoreFlashcardsRaw` with an exclusion clause appended to the prompt body, merges via `mergeUniqueByFront` (case-insensitive + whitespace-trimmed dedupe), writes the merged set back to `flashcards_<chapterId>` directly via `dbPut`). New `src/ui/flashcards-view.ts` (Loading / Error+Retry / Success states; success renders one card at a time with both faces in the DOM, `.flashcards__card--flipped` toggles a `rotateY(180deg)` transform; Prev/Next clamp at boundaries via `setDisabled` + `data-role="flashcards-{prev,next,more}"`; counter `n / N`; nav resets flipped state; "More cards" calls `appendFlashcards`, then `updateCardInPlace` mutates the existing DOM nodes — no re-render during user interaction, so flip stays smooth on mobile). `src/ui/chapter-view.ts` wires a third tab: Read | Summary | Flashcards. CSS in `src/styles.css`: `.flashcards__stage` owns the `perspective: 1200px`, `.flashcards__card` does the GPU-only `transform: rotateY` transition (`will-change: transform`, no layout-affecting properties → 60fps on mid-tier mobile, AC #2); `backface-visibility: hidden` on both faces; reserved `min-height: 14rem` so flipping never reflows neighbouring content (no CLS); `prefers-reduced-motion` kills the transition. `src/test/dom-stub.ts` extended with `removeAttribute` (real-DOM API missing from the stub, needed by `setDisabled`'s un-disable path). 29 new vitest tests: `src/ai/modes/flashcards.test.ts` (16 — generateFlashcardsRaw edge cases × 8 covering empty key, valid response, <5 enforcement, malformed JSON, missing cards, missing front/back, prompt substitution, strict schema; loadFlashcards cache integration × 3; appendFlashcards × 5 covering empty-cache fallthrough, append + merge order, case+whitespace dedup, writeback, and prompt-augmentation exclusion clause). `src/ui/flashcards-view.test.ts` (13 — render states × 4, showFlashcards orchestration × 8 covering no-key empty state, success render, click flip toggle, Prev/Next navigation, navigation resets flipped, More cards updates counter via `vi.waitFor`, API failure error state). Full suite: 344/344 (was 315). `tsc --noEmit` clean. `npm run build` clean (272 kB JS / 80 kB gz; 18.5 kB CSS / 3.9 kB gz). `bpsai-pair arch check` clean on all touched files. Open follow-up: real 60fps verification needs Lighthouse / a mid-tier device — the CSS architecture is designed for it but the measurement is deferred (same pattern as TA.3 CLS gate). AC #1 (≥ 5 cards), #2 (flip animation runs at 60fps — CSS-transform-only architecture verified, device-measurement deferred), #3 (More cards appends without duplicate fronts — case-insensitive + whitespace-trimmed dedupe), #4 (cache key `flashcards_<chapterId>` — locked via `generationKey`) all green.
